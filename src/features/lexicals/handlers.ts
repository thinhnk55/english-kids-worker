...
interface LexicalRow { id: string; text: string; tokens: string; pronunciation: string | null; translations: string }
...
export async function listLexicals(request: Request, env: Env, origin: string): Promise<Response> {
  try { const url = new URL(request.url); const { page, size, offset } = parsePagination(url); const q = url.searchParams.get('q')?.trim() ?? ''; const where = q ? 'WHERE text LIKE ?' : ''; const bind = q ? [`%${q}%`] : []; const count = await env.DB.prepare(`SELECT COUNT(*) total FROM lexicals ${where}`).bind(...bind).first<{ total: number }>(); const rows = await env.DB.prepare(`SELECT id, text, tokens, pronunciation, translations FROM lexicals ${where} ORDER BY id DESC LIMIT ? OFFSET ?`).bind(...bind, size, offset).all<LexicalRow>(); return successResponse(200, 'SUCCESS', rows.results.map(lexical), origin, { page, size, total: count?.total ?? 0 }) } catch (error) { return errorResponse(500, 'INTERNAL_ERROR', error instanceof Error ? error.message : undefined, origin) }
}
...
async function remove(env: Env, id: string): Promise<boolean> { const row = await find(env, id); if (!row) return false; const media = await env.DB.prepare(`SELECT url FROM ${audio} WHERE lexical_id = ? UNION ALL SELECT url FROM ${image} WHERE lexical_id = ? UNION ALL SELECT url FROM ${video} WHERE lexical_id = ?`).bind(id, id, id).all<{ url: string }>(); const base = env.ASSET_BASE_URL.replace(/\/$/u, ''); await Promise.all(media.results.filter(item => item.url.startsWith(`${base}/`)).map(item => env.ASSETS.delete(item.url.slice(base.length + 1)))); await env.DB.batch([env.DB.prepare(`DELETE FROM ${audio} WHERE lexical_id = ?`).bind(id), env.DB.prepare(`DELETE FROM ${image} WHERE lexical_id = ?`).bind(id), env.DB.prepare(`DELETE FROM ${video} WHERE lexical_id = ?`).bind(id), env.DB.prepare('DELETE FROM lexicals WHERE id = ?').bind(id)]); return true }
export async function deleteLexical(env: Env, origin: string, id: string): Promise<Response> { try { return await remove(env, id) ? successResponse(200, 'DELETED', undefined, origin) : errorResponse(404, 'NOT_FOUND', 'Không tìm thấy lexical', origin) } catch (error) { return errorResponse(500, 'INTERNAL_ERROR', error instanceof Error ? error.message : undefined, origin) } }
export async function batchDeleteLexicals(request: Request, env: Env, origin: string): Promise<Response> { const value = await body(request, origin); if (response(value) || !Array.isArray(value.ids)) return response(value) ? value : errorResponse(400, 'VALIDATION_ERROR', 'ids phải là mảng', origin); const ids = value.ids.filter((id): id is string => typeof id === 'string' && Boolean(id)); await Promise.all(ids.map(id => remove(env, id))); return successResponse(200, 'DELETED', { count: ids.length }, origin) }
