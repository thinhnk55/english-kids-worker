...
async function findInstruction(env: Env, id: string): Promise<InstructionMediaRow | null> {
  return env.DB.prepare(`
    SELECT id, engine, voice_id, audio
...
export async function createInstructionAudioPresign(request: Request, env: Env, origin: string, id: string): Promise<Response> {
  ...
  const key = instructionAudioKey(instruction.id, instruction.engine, instruction.voice_id);
  ...
  return successResponse(200, 'SUCCESS', {
    id: 'audio',
    key,
    url: instructionAudioUrl(env, instruction.id, instruction.engine, instruction.voice_id),
...
export async function confirmInstructionAudio(env: Env, origin: string, id: string): Promise<Response> {
  ...
  const audio = instructionAudioUrl(env, instruction.id, instruction.engine, instruction.voice_id);
  ...
  return successResponse(200, 'UPDATED', { id: 'audio', url: audio, engine: instruction.engine, voice_id: instruction.voice_id }, origin);
}
